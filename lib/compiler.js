  ...
  for (let lineNumber in lines) {
    ...
    if (
      ...
      if (!isUndefined(person)) {
        throw new Error(`But they have already met ${person}! *awkward silence*`);
      } else {
    ...
    /*if (new RegExp(`^What did we learn from this\? ${datatypes.any}${punctuationRegExp}$`).test(line)) {
      // Return the chapter's lesson!
      let lesson = removePunctuation(tokens[6]);

      if (new RegExp(`^${variable}$`).test(lesson)) {
        if (isUndefined(lesson)) {
          throw new Error(`The lesson must exist!`);
        }
      }

      output += `return ${lesson};\n`;
      continue;
    }*/
      ...
      let args = spec
        ...
        .map(arg => {
          ...
          if (new RegExp(`^${variable}$`).test(value) && isUndefined(value)) {
            throw new Error(`${value} does not exist, ya dummy!`);
          }
    ...
    if (
      ...
      if (isUndefined(varname)) {
        throw new Error(
          `${varname} can't become anything if they aren't born yet! Jeez!`
        );
      }
    ...
    function parseValue(value) {
      ...
      if (new RegExp(`^${variable}$`).test(value)) {
        if (isUndefined(value)) {
          throw new Error(`Whut? They haven't met ${value} yet!`);
        }
