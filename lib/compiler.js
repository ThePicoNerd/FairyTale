...
function compile({source}) {
  ...
  for (let lineNumber in lines) {
    ...
    if (new RegExp(`^Chapter [0-9]+ - ${variable}.*$`, "g").test(line)) {
      if (level !== 0) {
        throw new Error(`What kind of tomfoolery is this? There cannot be chapters inside of other chapters.`);
      }

      let chapterNumber = parseInt(tokens[1]);

      if (!/[0-9]/g.test(chapterNumber) || chapterNumber !== chapter + 1) {
        throw new Error(`This doesn't make any sense... The chapters are out of order!`);
      }
      ...
      let args = spec.filter(arg => {
        ...
        if (!new RegExp(`^${variable}$`).test(arg)) {
          throw new Error(`"${arg}" is not a valid character name! Would you like your child to be called that? No, didn't think so!`);
        }
    ...
    if (closingSentences.includes(line)) {
      if (level > 0) {
        ...
        throw new Error(`What have you done!? You cannot close more blocks than you open (more "}" than "{")!`)
      }
    }
